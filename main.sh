...
confirm_prompt "Install vscode?" && sudo sh -c "cat \"$DIR\"/files/vscode > /etc/yum.repos.d/vscode.repo" &&
    sudo yum install -y code
confirm_prompt "Install docker?" && sudo sh -c "cat \"$DIR\"/files/docker > /etc/yum.repos.d/docker-ce.repo" &&
    sudo yum install -y docker-ce docker-ce-cli containerd.io &&
    sudo systemctl enable docker.service
confirm_prompt "Install keepass2, and mono?" && sudo sh -c "cat \"$DIR\"/files/mono > /etc/yum.repos.d/mono-centos7-stable.repo" &&
    (
        test -d /opt/keepass2 && sudo rm -r /opt/keepass2
    ) &&
    ...
    (
        if verify_hash d414db9d411acbafd4e1b5faa8605ad7a28829f1814ce779142ffbd5ca9c3794 keepass2.zip
        then
            exit 0
        else
    ...
    sudo sh -c "cat \"$DIR\"/files/keepass2 > /usr/share/applications/keepass2.desktop" &&
...
confirm_prompt "Set shortcuts? Warning: overwrites existing shortcuts." && {
...
confirm_prompt "Build and install tmux 2.9?" && 
    ...
    (
        cat ~/.bashrc | grep -q "alias tm=\"tmux attach || tmux\""
        if test $? -ne 0
        then
